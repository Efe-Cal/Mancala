class Mangala():
    ...
    def play(self,start_index:int,player:int):
        ...
        if PLAYABLE_HOLE and CORRECT_PLAYER: # checks if the hole is playable
            addone=0
            ...
            for i in range(1,bkts:=self.tahta[player][start_index]+addone): # moves the stons
                ...
                match a := start_index+i:
                    ...
                    case a if 13>a>6:
                        ...
                        subtract=-7
                        if LAST_STONE: # checks if the situation in 2nd rule and enforces the rule
                            print("###")
                    ...
                    case a if 19>a>13:
                        ...
                        if LAST_STONE and REQ_FOR_3RD_RULE :
                            ...
                            self.tahta[player][start_index+i+subtract+addone]=0
                            self.tahta[0 if player else 1][5-(start_index+i+subtract+addone)]=0
                #print(f"{i == bkts-1} and self.tahta[player][{start_index}+{i}+{subtract}-1+{addone}] and {start_index+bkts-1+subtract+addone} and {self.tahta[0 if player else 1][5-(start_index+bkts-1+subtract+addone)]}")
################    # # [Following if block] checks if the situation in 3rd rule and enforces the rule
                # REQ_FOR_3RD_RULE=self.tahta[player][start_index+i+subtract+addone]==1 and start_index+i+subtract+addone!=6 and self.tahta[0 if player else 1][5-(start_index+i+subtract+addone)]!=0
                # if LAST_STONE and REQ_FOR_3RD_RULE :
                #     print("*****")
